...
void ForceFieldEAMDMD::calc_mu()
{
    ...
    /*--------------------------------
     ...
     here
     --------------------------------*/
    int nall=natms+atoms->natms_ph;
    for(int i=natms;i<nall;i++)
    {
        type0 cv_i=1.0;
        for(int j=0;j<c_dim;j++)
            if(c[i*c_dim+j]>0.0)
    ...
    for(int i=0;i<n;i++)
    {
        ...
        for(int j,__j=0;__j<neigh_sz;__j++,istart+=3)
        {
            ...
            if(j<n)
            {
                mu[i]+=c[i]*(rho_phi[istart+0]+rho_phi[istart+2]*dE[i]);
...
type0 ForceFieldEAMDMD::update_J(type0 __alpha_tmp,type0* a,type0* g)
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        for(int j,__j=0;__j<neigh_sz;__j++,istart+=3)
        {
            ...
            M_IJ[istart]=kbT*(d_i*theta_i*exp_mod_Qi*c[i]*cv[j/c_dim]-d_j*theta_j*exp_mod_Qj*c[j]*cv[i/c_dim]);
