...
/*--------------------------------------------
 ...
 --------------------------------------------*/
template<const int M>
Neighbor::Cell<M>::Cell(Atoms*& __atoms):
atoms(__atoms),
nneighs(neigh_size()),
head_atm(NULL)
{
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
template<const int M>
int Neighbor::Cell<M>::neigh_size()
{
    int countr[__dim__]={[0 ... __dim__-1]=-M};
    constexpr int max_nneighs=Algebra::pow<2*M+1,__dim__>();
    constexpr int rc_sq=M*M;
...
void Neighbor::Cell<M>::DO(bool box_chng,F0 pre,F1 mid,F2 post)
{
    if(box_chng) setup();
