...
/*--------------------------------------------
 ...
 --------------------------------------------*/
ForceFieldEAMDMDSCC::
ForceFieldEAMDMDSCC(AtomsDMD* atoms,
...
/*--------------------------------------------
 destructor
 --------------------------------------------*/
ForceFieldEAMDMDSCC::~ForceFieldEAMDMDSCC()
{
...
/*--------------------------------------------
 force calculation
 --------------------------------------------*/
void ForceFieldEAMDMDSCC::calc_pair(type0 r,type0* alpha_i,type0* alpha_j,type0*& __rho_phi,type0*&,type0*&)
{}
/*--------------------------------------------
 force calculation
 --------------------------------------------*/
void ForceFieldEAMDMDSCC::force_calc()
{
    ...
    for(int i=0;i<natms_lcl;i++)
    {
        ...
        for(int j,__j=0;__j<neigh_sz;__j++)
        {
            ...
            //rsq=Algebra::RSQ<__dim__>(x+i*__dim__,x+j*__dim__)/(atoms->H[0][0]*atoms->H[0][0]);
            type0 pair_fac=1.0-delta*(c[i*c_dim]-c[j*c_dim])*(c[i*c_dim]-c[j*c_dim]);
            ...
            if(j<natms_lcl)
            {
                for(int ic=0;ic<c_dim;ic++)
                    ...
                    {
                        fpair-=(drho_phi_dr[istart+2]*dE[i*c_dim+ic]+drho_phi_dr[istart+1]*dE[j*c_dim+jc]+drho_phi_dr[istart])*c[i*c_dim+ic]*c[j*c_dim+jc]*pair_fac;
                        apair=-(drho_phi_dalpha[istart+2]*dE[i*c_dim+ic]+drho_phi_dalpha[istart+1]*dE[j*c_dim+jc]+drho_phi_dalpha[istart])*c[i*c_dim+ic]*c[j*c_dim+jc]*pair_fac;
...
/*--------------------------------------------
 energy calculation
 --------------------------------------------*/
void ForceFieldEAMDMDSCC::energy_calc()
{
    if(max_pairs<neighbor->no_pairs)
    {
        delete [] rho_phi;
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void ForceFieldEAMDMDSCC::sc_loop()
{
    ...
    for(int i=0;i<natms_lcl;i++)
    {
        ...
        for(int j,__j=0;__j<neigh_sz;__j++)
        {
            ...
            type0 dc_sq=(c[i*c_dim]-c[j*c_dim])*(c[i*c_dim]-c[j*c_dim]);
            if(j<natms_lcl)
            {
                for(int ic=0;ic<c_dim;ic++)
                    for(int jc=0;jc<c_dim;jc++,istart+=3,iistart++)
                    {
                        __vec_lcl[0]+=(1.0-delta*dc_sq)*B_pair[iistart]*rho_phi[istart];
                        if(c[i*c_dim+ic]>0.0)
                            rho[i*c_dim+ic]+=(1.0-delta*dc_sq)*B_pair[iistart]*rho_phi[istart+2]/c[i*c_dim+ic];
                        if(c[j*c_dim+jc]>0.0)
                            rho[j*c_dim+jc]+=(1.0-delta*dc_sq)*B_pair[iistart]*rho_phi[istart+1]/c[j*c_dim+jc];
            ...
            else
            {
                for(int ic=0;ic<c_dim;ic++)
                    for(int jc=0;jc<c_dim;jc++,istart+=3,iistart++)
                    {
                        __vec_lcl[0]+=0.5*(1.0-delta*dc_sq)*B_pair[iistart]*rho_phi[istart];
                        if(c[i*c_dim+ic]>0.0)
                            rho[i*c_dim+ic]+=(1.0-delta*dc_sq)*B_pair[iistart]*rho_phi[istart+2]/c[i*c_dim+ic];
...
/*--------------------------------------------
 force calculation
 --------------------------------------------*/
void ForceFieldEAMDMDSCC::prep(Vec<type0>*,Vec<type0>*)
{}
/*--------------------------------------------
 
 --------------------------------------------*/
void ForceFieldEAMDMDSCC::J(VecTens<type0,2>& Dx,VecTens<type0,2>& ADx)
{}
/*--------------------------------------------
 init
 --------------------------------------------*/
void ForceFieldEAMDMDSCC::init()
{
    pre_init();
    set_temp();
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void ForceFieldEAMDMDSCC::set_temp()
{
    type0 T=atoms->temp;
    type0 kb=atoms->kB;
    type0 hP=atoms->hP;
...
/*--------------------------------------------
 return M_{ij}^{\alpha}
 --------------------------------------------*/
inline type0 ForceFieldEAMDMDSCC::calc_ent(type0 x)
{
    if(x==0.0) return 0.0;
    return x*log(x);
}
/*--------------------------------------------
 python constructor
 --------------------------------------------*/
void ForceFieldEAMDMDSCC::ml_new(PyMethodDef& method_0,PyMethodDef& method_1,PyMethodDef& method_2)
{
    ...
    [](PyObject* self,PyObject* args,PyObject* kwds)->PyObject*
    {
        ...
        FuncAPI<std::string*,type0*,type0*,std::string*> f("ff_eam_funcfl_scc",{"funcfl_files","r_crd","C","elems"});
        f.noptionals=1;
    ...
    {
        AtomsDMD::Object* __self=reinterpret_cast<AtomsDMD::Object*>(self);
        FuncAPI<std::string,type0*,type0*,std::string*> f("ff_eam_setfl_scc",{"setfl_file","r_crd","C","elems"});
        f.noptionals=1;
        f.logics<1>()[0]=VLogics("gt",0.0);
        f.logics<2>()[0]=VLogics("gt",0.0);
    ...
    {
        AtomsDMD::Object* __self=reinterpret_cast<AtomsDMD::Object*>(self);
        FuncAPI<std::string,type0*,type0*,std::string*> f("ff_eam_fs_scc",{"fs_file","r_crd","C","elems"});
        f.noptionals=1;
        f.logics<1>()[0]=VLogics("gt",0.0);
        f.logics<2>()[0]=VLogics("gt",0.0);
