...
const type0 __t_relax,const int __nchains,
const int __niters):
t_relax(__t_relax),
...
{
    eta_d=new type0[nchains];
    for(int ich=0;ich<nchains;ich++) eta_d[ich]=0.0;
}
/*--------------------------------------------
...
type0 ThermostatNHC::operator()(type0 T_r,int ndof)
{
    ...
    for(int iter=0;iter<niters;iter++)
    {
        eta_d[nchains-1]+=ddt2*(eta_d[nchains-2]*eta_d[nchains-2]-freq_sq);
        prev_fac=-eta_d[nchains-1];
        
        for(int ich=nchains-2;ich>0;ich--)
        {
    ...
    for(int iter=0;iter<niters;iter++)
    {
        eta_d[nchains-1]+=(eta_d[nchains-2]*eta_d[nchains-2]-freq_sq)*ddt2;
        prev_fac=exp(-ddt4*eta_d[nchains-1]);
