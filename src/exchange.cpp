...
/*--------------------------------------------
 ...
 --------------------------------------------*/
inline void __Update::add_to_snd_lst(int& icomm,int& iatm)
{
    if(snd_atms_lst_sz[icomm]+1>snd_atms_lst_cpcty[icomm])
    {
        int* tmp_lst=new int[snd_atms_lst_sz[icomm]+1+snd_atms_lst_grw];
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
inline void __Update::reserve_rcv_buff(int xtra)
{
    if(rcv_buff_cpcty<xtra+rcv_buff_sz)
    {
        delete [] rcv_buff;
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
inline void __Update::reserve_snd_buff(int xtra)
{
    if(snd_buff_cpcty<xtra+snd_buff_sz)
    {
        delete [] snd_buff;
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void __Update::update(vec** updt_vecs,int nupdt_vecs,bool x_xst)
{
    int tot_byte_sz=0;
    for(int ivec=0;ivec<nupdt_vecs;ivec++)
    {
    ...
    for(int idim=0;idim<__dim__;idim++)
    {
        for(int idir=0;idir<2;idir++)
        {
            ...
            {
                if(self_comm[idim])
                    self_update_mult(icomm,neigh[idim][idir],neigh[idim][1-idir],updt_vecs,nupdt_vecs,tot_byte_sz);
                else
                    update_mult(icomm,neigh[idim][idir],neigh[idim][1-idir],updt_vecs,nupdt_vecs,tot_byte_sz);
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void __Update::update(vec* updt_vec,bool x_xst)
{
    snd_buff_sz=0;
    reserve_snd_buff(updt_vec->byte_sz*max_snd_atms_lst_sz);
    updt_vec->vec_sz=natms_lcl;
    ...
    for(int idim=0;idim<__dim__;idim++)
    {
        for(int idir=0;idir<2;idir++)
        {
            ...
            {
                if(self_comm[idim])
                    self_update_sing(icomm,neigh[idim][idir],neigh[idim][1-idir],updt_vec);
                else
                    update_sing(icomm,neigh[idim][idir],neigh[idim][1-idir],updt_vec);
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void __Update::update(vec* updt_vec,type0 (*dH)[__dim__])
{
...
/*------------------------------------------------------------------------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------------------------------------------------------------------*/
inline void __Update::load_unload
(int& __icomm,int& __snd_p,int& __rcv_p)
{
    snd_buff_sz=snd_atms_lst_sz[__icomm]*tot_updt_vecs_sz;
    if(snd_buff_cpcty<snd_buff_sz)
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
inline void __Update::update_mult
(int& __icomm,int& __snd_p,int& __rcv_p,vec**& __vecs
,int& __nvecs,int& __vecs_byte_sz)
{
    byte* tmp_snd_buff=snd_buff;
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
inline void __Update::update_sing
(int& __icomm,int& __snd_p,int& __rcv_p,vec*& __v)
{
    byte* tmp_snd_buff=snd_buff;
    __v->cpy(tmp_snd_buff,snd_atms_lst[__icomm],snd_atms_lst_sz[__icomm]);
...
/*------------------------------------------------------------------------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------------------------------------------------------------------*/
inline void __Update::self_load_unload
(int& __icomm,int&,int&)
{
    rcv_atms_lst_sz[__icomm]=snd_atms_lst_sz[__icomm];
