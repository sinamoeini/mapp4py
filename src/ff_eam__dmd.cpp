...
/*--------------------------------------------
 ...
 --------------------------------------------*/
ForceFieldEAM_DMD::
ForceFieldEAM_DMD(AtomsDMD* atoms,
...
/*--------------------------------------------
 destructor
 --------------------------------------------*/
ForceFieldEAM_DMD::~ForceFieldEAM_DMD()
{
...
/*--------------------------------------------
 force calculation
 --------------------------------------------*/
void ForceFieldEAM_DMD::force_calc()
{
...
/*--------------------------------------------
 force calculation
 --------------------------------------------*/
type0 ForceFieldEAM_DMD::prep(VecTens<type0,2>& f)
{
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void ForceFieldEAM_DMD::J(VecTens<type0,2>& Dx,VecTens<type0,2>& ADx)
{
...
/*--------------------------------------------
 energy calculation
 --------------------------------------------*/
void ForceFieldEAM_DMD::energy_calc()
{
    type0* rho=ddE_ptr->begin();
    const int n=atoms->natms_lcl*c_dim;
    for(int i=0;i<n;i++) rho[i]=0.0;
    ...
    for(int i=0;i<n;i++)
    {
        ...
        if(c[i]!=0.0)
            __vec_lcl[0]+=c[i]*(tmp1+c_0[elem_i]-3.0*kbT*log(alpha[i]));
        __vec_lcl[0]+=kbT*calc_ent(c[i]);
    }
    const int __natms=atoms->natms_lcl;
    type0 cv_i;
    for(int i=0;i<__natms;i++)
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void ForceFieldEAM_DMD::init_static()
{
...
/*--------------------------------------------
 create the sparse matrices
 --------------------------------------------*/
void ForceFieldEAM_DMD::fin_static()
{
    delete fcoef_ptr;
    delete vec3;
    delete vec2;
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void ForceFieldEAM_DMD::set_temp()
{
    type0 T=atoms->temp;
    type0 kb=atoms->kB;
    type0 hP=atoms->hP;
...
/*--------------------------------------------
 return M_{ij}^{\alpha}
 --------------------------------------------*/
inline type0 ForceFieldEAM_DMD::calc_ent(type0 x)
{
    if(x==0.0) return 0.0;
    return x*log(x);
}
/*--------------------------------------------
 ...
 mu_ptr;
 --------------------------------------------*/
void ForceFieldEAM_DMD::calc_mu()
{
    /*--------------------------------
...
/*--------------------------------------------
 force calculation
 --------------------------------------------*/
void ForceFieldEAM_DMD::force_calc_static()
{
    int n=atoms->natms_lcl*c_dim;
    ...
    {
        if(i%c_dim==0)
            __vec_lcl[0]+=kbT*calc_ent(cv[i]);
        if(c[i]>=0.0)
            __vec_lcl[0]+=c[i]*(E[i]+c_0[elem[i]]-3.0*kbT*log(alpha[i]))+kbT*calc_ent(c[i]);
    }
...
/*--------------------------------------------
 ...
 F=c-scalar*c_d(c)+a
 --------------------------------------------*/
void ForceFieldEAM_DMD::c_d_calc()
{
    calc_mu();
...
/*--------------------------------------------
 ...
 Ax_i=-J_ij*x_j;
 --------------------------------------------*/
void ForceFieldEAM_DMD::J(Vec<type0>* x_ptr,Vec<type0>* Ax_ptr)
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        {
            j=neighbor_list[i][__j];
            b0[i]+=(0.5*rho_phi[istart]+dE[i]*rho_phi[istart+2])*x[j];
            if(j<n)
                b0[j]+=(0.5*rho_phi[istart]+dE[j]*rho_phi[istart+1])*x[i];
        }
    }
...
/*--------------------------------------------
 python constructor
 --------------------------------------------*/
void ForceFieldEAM_DMD::ml_new(PyMethodDef& method_0,PyMethodDef& method_1,PyMethodDef& method_2)
{
    method_0.ml_flags=METH_VARARGS | METH_KEYWORDS;
    method_0.ml_name="ff_eam__funcfl";
    ...
    [](PyObject* self,PyObject* args,PyObject* kwds)->PyObject*
    {
        ...
        FuncAPI<std::string*,type0*,type0*,std::string*> f("ff_eam__funcfl",{"funcfl_files","r_crd","zeta","elems"});
        f.noptionals=1;
        ...
        delete __self->ff;
        __self->ff=new ForceFieldEAM_DMD(__self->atoms,dr,drho,nr,nrho,std::move(r_phi),std::move(rho),std::move(F),std::move(r_c),f.mov<1>(),f.mov<2>());
        Py_RETURN_NONE;
    };
    ...
    method_1.ml_name="ff_eam__setfl";
    method_1.ml_meth=(PyCFunction)(PyCFunctionWithKeywords)
    [](PyObject* self,PyObject* args,PyObject* kwds)->PyObject*
    {
        AtomsDMD::Object* __self=reinterpret_cast<AtomsDMD::Object*>(self);
        FuncAPI<std::string,type0*,type0*,std::string*> f("ff_eam__setfl",{"setfl_file","r_crd","zeta","elems"});
        f.noptionals=1;
        f.logics<1>()[0]=VLogics("gt",0.0);
        f.logics<2>()[0]=VLogics("gt",0.0);
    ...
    method_2.ml_name="ff_eam__fs";
    method_2.ml_meth=(PyCFunction)(PyCFunctionWithKeywords)
    [](PyObject* self,PyObject* args,PyObject* kwds)->PyObject*
    {
        AtomsDMD::Object* __self=reinterpret_cast<AtomsDMD::Object*>(self);
        FuncAPI<std::string,type0*,type0*,std::string*> f("ff_eam__fs",{"fs_file","r_crd","zeta","elems"});
        f.noptionals=1;
        f.logics<1>()[0]=VLogics("gt",0.0);
        f.logics<2>()[0]=VLogics("gt",0.0);
